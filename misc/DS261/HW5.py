...
# MAGIC %md
# MAGIC ## HW 5 - Page Rank - EXTRA CREDIT
# MAGIC __`MIDS w261: Machine Learning at Scale | UC Berkeley School of Information | Fall 2025`__
# MAGIC
# MAGIC > Updated: 11/24/2025
# MAGIC
...
################ YOUR CODE HERE #################
row_sums = np.sum(TOY_ADJ_MATR, axis=1)
transition_matrix = TOY_ADJ_MATR.div(row_sums, axis=0) # replace with your code
################ (END) YOUR CODE #################
...
def power_iteration(xInit, tMatrix, nIter, verbose = True):
    ...
    state_vector = None
    ################ YOUR CODE HERE #################
    state_vector = xInit.copy()
    current_matrix = tMatrix.copy()
    for step in range(nIter):
        state_vector = state_vector @ current_matrix
...
trans_matr = adj_matr.div(np.sum(adj_matr, axis=1), axis=0).fillna(0) # replace with your code
xInit = np.array([1.0, 0, 0, 0, 0])
state = power_iteration(xInit, trans_matr, 10) # replace with your code
################ (END) YOUR CODE #################
...
def count_nodes(dataRDD):
    ...
    """    
    ############## YOUR CODE HERE ###############
    # Get all source nodes (nodes that have entries in the file)
    source_nodes = dataRDD.map(lambda line: line.split('\t')[0])

    # Get all destination nodes (nodes that are targets of edges)
    def extract_destinations(line):
        """Extract destination node IDs from the adjacency dictionary."""
        parts = line.split('\t')
        # Parse the dictionary string to extract destination node IDs
        adj_dict = ast.literal_eval(parts[1])
        return list(adj_dict.keys())

    destination_nodes = dataRDD.flatMap(extract_destinations)

    # Concatenate all nodes and count distinct
    all_nodes = source_nodes.union(destination_nodes)
    totalCount = all_nodes.distinct().count()
    ############## (END) YOUR CODE ###############   
    return totalCount
...
############## YOUR CODE HERE ###############
num_dangling = tot - 5781290 # your code here
############## (END) YOUR CODE ###############   
...
def count_degree(dataRDD, n):
    ...
    ############## YOUR CODE HERE ###############
    # Step 1: Parse each line and sum the weights for each node as out-degree
    node_degree_pairs = dataRDD.map(parse).map(lambda x: (x[0], sum(x[1].values())))

    # Cache this RDD as we'll use it multiple times
    node_degree_pairs.cache()

    # Step 2: Sort by degree in descending order and take top 10
    top = node_degree_pairs.takeOrdered(10, key=lambda x: -x[1])

    # Step 3: Calculate average out-degree for non-dangling nodes
    total_degree = node_degree_pairs.map(lambda x: x[1]).sum()
    total_nodes = node_degree_pairs.count()
    avgDegree = total_degree / total_nodes if total_nodes > 0 else 0.0

    # Step 4: Take a sample of n out-degree counts
    # Use sample() to randomly sample approximately n records
    sample_fraction = min(1.0, n / total_nodes) if total_nodes > 0 else 1.0
    sampledCounts = node_degree_pairs.sample(False, sample_fraction, seed=42) \
        .map(lambda x: x[1]) \
        .take(n)
    ############## (END) YOUR CODE ###############
...
# MAGIC
# MAGIC > __d)__ Numeric Answer: 151 
# MAGIC
...
# MAGIC
# MAGIC > __c)__ Numeric Answer: 1.0
# MAGIC
# MAGIC > __e)__ Highlight the correct answer (Add ** before and after your selected choice):
# MAGIC
# MAGIC * 1184351
...
def initGraph(dataRDD):
    ...
    def parse_and_convert(line):
        # the new format is faster to parse than json string
        parts = line.split('\t')
        source_node = int(parts[0])
        adj_dict = ast.literal_eval(parts[1])
        # Convert to efficient format: "dest:weight,dest:weight,..."
        edges_str = ','.join([f"{dest}:{weight}" for dest, weight in adj_dict.items()])
        return source_node, edges_str

    # Parse dataRDD and convert to efficient format
    source_nodes = dataRDD.map(parse_and_convert)

    # Step 1: Get all destination nodes (for identifying dangling nodes)
    all_destinations = source_nodes.flatMap(
        lambda x: [(int(dest.split(':')[0]), '') for dest in x[1].split(',') if x[1]]
    )

    # Step 2: Union source nodes with destination nodes, keeping edges for source nodes
    all_nodes_with_edges = source_nodes.union(all_destinations)

    # Step 3: Reduce by key to keep edges for source nodes, empty string for dangling nodes
    nodes_with_edges = all_nodes_with_edges.reduceByKey(lambda edges1, edges2: edges1 if edges1 else edges2)

    # Step 4: Calculate total number of nodes for initialization (N)
    N = nodes_with_edges.count()
    initial_score = 1.0 / N

    graphRDD = nodes_with_edges.map(lambda x: (x[0], (initial_score, x[1])))
    ############## (END) YOUR CODE ##############
...
def runPageRank(graphInitRDD, alpha = 0.15, maxIter = 10, verbose = True):
    ...
    def parseEdges(edges_str):
        """
        Parse edge string into dictionary.

        @param edges_str: edge string in format "dest:weight,dest:weight,..." or empty string
        @return: dictionary of {dest: weight}, or empty dict if no edges
        """
        if not edges_str:
            return {}
        result = {}
        for edge in edges_str.split(','):
            if edge:
                ...
                result[int(dest)] = int(weight)
        return result

    def computeContribs(node_edges_rank):
        """Calculates contributions to the rank of neighbor nodes based on edge weights."""
        node_id, (edges_str, rank) = node_edges_rank

        # Parse edges using helper function - returns dict
        edges_dict = parseEdges(edges_str)

        if not edges_dict:
            # Dangling node - return empty contributions (mass will be accumulated separately)
            return []

        # Calculate total weight
        total_weight = sum(edges_dict.values())

        # Distribute rank proportionally to edge weights
        contributions = [(dest, rank * weight / total_weight) for dest, weight in edges_dict.items()]
        return contributions

    def accumulateDanglingMass(node_edges_rank):
        """Accumulate dangling mass using mmAccum (must be called in an action like foreach)."""
        node_id, (edges_str, rank) = node_edges_rank
        if not edges_str or edges_str.strip() == '':
            mmAccum.add(rank)

    def accumulateTotalMass(node_rank):
        """Accumulate total mass using totAccum (must be called in an action like foreach)."""
        node_id, rank = node_rank
        totAccum.add(rank)

    def updateRank(rank, total_dangling_mass, N):
        """Update rank with teleportation and dangling mass redistribution."""
        return a.value / N + d.value * (rank + total_dangling_mass / N)
    ...
    # Separate links (edges) and ranks - keep all nodes (including dangling nodes)
    links = graphInitRDD.map(lambda x: (x[0], x[1][1])).cache()
    ranks = graphInitRDD.map(lambda x: (x[0], x[1][0])).cache()

    # Get all node IDs from the original graph once (includes dangling nodes)
    all_node_ids = graphInitRDD.map(lambda x: (x[0], 0.0)).cache()

    for iteration in range(maxIter):
        # Reset dangling mass accumulator for this iteration
        mmAccum.value = 0.0

        # Join links and ranks - cache since used twice (for dangling mass and contributions)
        links_ranks = links.join(ranks).cache()

        # Use foreach action to accumulate dangling mass (avoids duplication)
        links_ranks.foreach(accumulateDanglingMass)
        dangling_mass_iter = mmAccum.value

        # Calculate contributions from non-dangling nodes
        contribs = links_ranks.flatMap(computeContribs)
        contrib_ranks = contribs.reduceByKey(add)

        # Left outer join to ensure all nodes are present, then update ranks
        ranks = all_node_ids.leftOuterJoin(contrib_ranks).mapValues(
            lambda x: updateRank(x[1] if x[1] is not None else 0.0, dangling_mass_iter, N)
        ).cache()

        # Unpersist the cached links_ranks to free memory
        links_ranks.unpersist()

        # Verbose logging - use foreach action to accumulate total mass
        if verbose:
            totAccum.value = 0.0  # Reset before accumulating
            ranks.foreach(accumulateTotalMass)
            total_mass = totAccum.value
            print(f'Iteration {iteration + 1}: Total Mass = {total_mass:.6f}, Dangling Mass = {dangling_mass_iter:.6f}')
